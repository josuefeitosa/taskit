  ...
  async update({ request, response, auth, params }) {
    ...
    if (isUserInTask === true || isUserInTask >= 0) {
      ...
      for (const teamMate of teamMates) {
        ...
        const indexExistingUser = taskJSON.users.findIndex(
          user => user.id === teamMate,
        );

        const teamMateUser = await User.findOrFail(teamMate);
        ...
        if (indexExistingUser < 0) {
          Mail.send('emails.newTask', dataForEmail, message => {
            message
              .to(teamMateUser.email)
              .from('noreplay.taskit@gmail.com')
              .subject('TaskIt - Nova tarefa para você!');
          });
        }
      ...
      for (const deletedTeamMate of taskJSON.users) {
        const dataForEmail = {
        ...
        if (!teamMates.includes(deletedTeamMate.id)) {
          Mail.send('emails.removedFromTask', dataForEmail, message => {
            message
              .to(deletedTeamMate.email)
              .from('noreplay.taskit@gmail.com')
              .subject('TaskIt - Atualização de tarefa');
          });
        }
      }
